...
//////////////////////////////////////////////////////////////////////////
/** @file
	@brief The unit-test of "util.hpp".
...
namespace
{
	...
	template<typename T>
		bool xxx_test(T)
		{
			...
			{
				if (dummy_clp2(x) != util::clp2(x))
					return false;
				if (dummy_flp2(x) != util::flp2(x))
					return false;
				if (dummy_parity(x) != util::parity(x))
					return false;
			}
...
#define TEST(expr) if (!(expr)) { os << "expression failed: \"" \
	...
	{
		os << std::setw(8) << i
			<< std::setw(8) << util::clp2(i)
			<< std::setw(8) << util::flp2(i)
			<< std::setw(8) << util::parity(i)
			<< "\n";
	}
	os << "\n";
